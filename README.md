...
**PromptOn is config-fetch, not a proxy.** This SDK fetches a *snapshot* of your pins, resolves and
...
sees your provider key, and if it goes down your app keeps generating on the last snapshot it
received.

```
your app ──resolve locally──▶ prompt + model + params        (snapshot, cached 10 s, ETag-polled)
   │
...
const resolution = prompton.resolve("greeting");                  // from memory, no network call
const messages = prompton.renderChat(resolution, { name: "Ada" }); // your variables, rendered locally

const answer = await prompton.withGeneration(resolution, { variables: { name: "Ada" } }, () =>
  openai.chat.completions.create({ model: resolution.model, messages, ...resolution.params }),
);
```

`withGeneration` times the call, builds the monitoring log and queues it. Whatever your function
...
| `project` | `PTN_PROJECT` | parsed from the API key | Names the disk-cache file and guards against a snapshot from another project |
| `cacheTtlMs` | — | `10000` | How long a snapshot is served without revalidating, and the poll interval |
| `requestTimeoutMs` | — | `5000` | Per-request timeout |
| `initialFetchTimeoutMs` | — | `3000` | Timeout of the first fetch, which never blocks a generation |
...
| `bundlePath` | `PTN_BUNDLE` | none | A snapshot file committed into the app, used when memory and disk are empty |
...
const prompton = new PromptOn({
  ...
  bundlePath: new URL("./prompton/snapshot.production.json", import.meta.url).pathname,
  hashEndUser: true,
});
```
...
This is the part that matters. A generation must never fail because PromptOn did.

**One snapshot, cached, polled.** `resolve()` is synchronous and reads memory. Inside the cache TTL
...
already holds. A refresh never blocks a generation and never fails one.

**Three tiers, in this order.** Memory, then a disk cache, then a bundle committed into the app:

| Tier | When it is used | `resolution.source` |
|---|---|---|
| Memory | Always, for every resolve | `remote` after a successful fetch |
...
prompton.exportSnapshot("prompton/snapshot.production.json");
```

Export one file per environment (`snapshot.production.json`, `snapshot.staging.json`) and point
...
| `429` with `Retry-After` | Waits it out before contacting the server again, keeps serving | Nothing. `snapshotInfo().stale` turns `true` |
| `5xx`, timeout, DNS failure, connection refused | Backs off ×2 from the cache TTL up to 5 minutes, keeps serving | Nothing |
| A snapshot for the wrong environment or project | Refuses it, keeps polling | Nothing, plus one warning line |
| PromptOn unreachable **and** nothing cached anywhere | — | `NotReadyError`: "PromptOn is unreachable and nothing is cached" |
| Use case not in the snapshot | — | `UnknownUseCaseError` |
| Use case has no live deployment here | — | `UnresolvedError`. Fix the deployment; **never** fall back to a hard-coded prompt |
| Prompt name is not pinned | — | `UnknownPromptError`, carrying `availablePrompts`. There is no silent fallback to `default` |
...
```ts
const resolution = prompton.resolve("diary_generation", { prompt: "ko" });
```
...
| Field | Meaning |
|---|---|
| `useCase`, `kind` | The key you asked for; `chat`, `text` or `embedding` |
| `prompt`, `availablePrompts` | The chosen prompt name (`null` for an embedding use case) and every name this revision pins |
| `deploymentId`, `deploymentRevision` | The pin that produced this |
| `promptVersionId`, `promptVersionNumber`, `engine` | The pinned, immutable prompt version |
...
| `messages` / `textTemplate` | The raw template, before rendering |
| `source`, `etag` | `remote` / `disk` / `bundle` / `manual`, and the snapshot's ETag |

Rendering is a separate step, because resolve is per deployment and render is per call:

```ts
const messages = prompton.renderChat(resolution, { transcriptions, mode: "fresh" });
const text = prompton.renderText(prompton.resolve("summarize"), { items });
```
...
`resolveRemote()` calls `POST /resolve`, which does the same thing on the server. It is the smoke
...
```ts
prompton.log(record, { resolution });   // queue a record you built yourself, returns immediately
await prompton.flush();                 // send now and wait — shutdown, tests, scripts
await prompton.withGeneration(resolution, meta, call);   // time a provider call and log it
```
...
| `params` | Layered over the resolution's params, for what was actually sent |
| `id` | A UUIDv7 you pre-issued with `prompton.generationId()` |

`extractOutcome` maps your provider's response onto `{ content, toolCalls, finishReason, usage:
...
| `use_case`, `kind`, `model`, `model_id`, `provider` | The resolution |
| `deployment_id`, `deployment_revision`, `prompt`, `prompt_version_id` | The resolution — this is the evidence that ties an answer to a pin |
...
| `latency_ms`, `started_at` | Measured by `withGeneration` |
| `trace_id`, `sequence`, `end_user_ref`, `context`, `metadata` | You |
| `sdk` | `{name: "prompton-nodejs", version}` |

**Payload policy.** Each use case carries one in the snapshot, and the SDK applies it *before* the
...
prompton.loadSnapshot(require("./fixtures/snapshot.json"));

const resolution = prompton.resolve("greeting");
await prompton.withGeneration(resolution, {}, () => ({ content: "hi", finishReason: "stop" }));

expect(prompton.logs[0].stop_kind).toBe("stop");   // captured, never sent
```

`mode: "offline"` resolves from the disk cache and the bundle and makes no HTTP call — for CI, for
local development, and for proving the fallback path works.

## Housekeeping

```ts
prompton.snapshotInfo();   // { etag, source, environment, project, fetchedAt, stale, ageSeconds }
